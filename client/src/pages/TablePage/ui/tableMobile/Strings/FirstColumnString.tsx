import { toggleModal } from "../../../../../lib/features/modalToggleSlice";
...
const IdFunctionalBlock = ({ func, name, id }: { func: boolean, name:string, id:string }) => {
        ...
        <>  
            {
                ...
                <>
                    ...
                    <svg onClick={handleModalData} className="absolute left-5" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
                ...
                <>
                    <svg style={{ border: "2px solid #942D2A" }} className="w-4 h-4 absolute left-5" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" fill="none">
...
export default function FirstStringMobile({name, death, isAlive, activeIcons, funcActive, id}:{ name:string, death:number, isAlive:boolean, activeIcons:boolean, funcActive:boolean, id:string}) {
    return(
        <>      
            <div className={`w-full flex h-24 ${isAlive ? 'text-[#FF6054]' : 'text-[#942D2A]'}`}>
                <div style={{ width: "50%", borderBottom: "2px solid #942D2A", borderRight: "2px solid #942D2A" }} className={`text-l flex justify-center place-items-center relative h-full ${isAlive ? 'text-[#FF6054]' : 'text-[#942D2A]'}`}>       
                    {
                            isAlive ? 
                            <>
                                {
                                    funcActive ?  <IdFunctionalBlock func={activeIcons} name={name} id={id}/> : ""
                                }
                            </>
                            ...
                            <>
                                {
                                    funcActive ? 
                                    <IdFunctionalBlock func={activeIcons} name={name} id={id}/> :
                                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="32" viewBox="0 0 28 32" fill="none" className="absolute left-5">
